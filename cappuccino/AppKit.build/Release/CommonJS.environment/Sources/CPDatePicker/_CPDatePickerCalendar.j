@STATIC;1.0;i;11;CPControl.ji;13;CPTextField.ji;10;CPButton.ji;8;CPView.ji;14;CPBezierPath.ji;20;_CPDatePickerClock.jI;23;Foundation/Foundation.jt;57174;objj_executeFile("CPControl.j", YES);objj_executeFile("CPTextField.j", YES);objj_executeFile("CPButton.j", YES);objj_executeFile("CPView.j", YES);objj_executeFile("CPBezierPath.j", YES);objj_executeFile("_CPDatePickerClock.j", YES);objj_executeFile("Foundation/Foundation.j", NO);var CPShortWeekDayNameArrayEn = ["Mo", "Tu", "We", "Th", "Fr", "Sa", "Su"],
...
,["void"]), new objj_method(sel_getUid("_clickArrowNext:"), function $_CPDatePickerCalendar___clickArrowNext_(self, _cmd, sender)
{
    ...
    {
        var date = objj_msgSend(objj_msgSend(self._monthView, "monthDate"), "copy");
        date.setDate(1);
        if (modifierFlags & CPAlternateKeyMask)
            date.setUTCFullYear(date.getUTCFullYear() + 10);
        else
            date.setUTCFullYear(date.getUTCFullYear() + 1);
        objj_msgSend(self, "setDateValue:", date);
    }
    else
    {
        objj_msgSend(self, "_displayNextMonth");
    }
}
...
,["void"]), new objj_method(sel_getUid("_clickArrowPrevious:"), function $_CPDatePickerCalendar___clickArrowPrevious_(self, _cmd, sender)
{
    ...
    {
        objj_msgSend(self, "_displayPreviousMonth");
    }
}
...
class_addMethods(the_class, [new objj_method(sel_getUid("initWithFrame:datePicker:delegate:"), function $_CPDatePickerHeaderView__initWithFrame_datePicker_delegate_(self, _cmd, aFrame, aDatePicker, aDelegate)
{
    ...
    if (self)
    {
        ...
        objj_msgSend(self._previousButton, "setAction:", sel_getUid("_clickArrowPrevious:"));
        objj_msgSend(self._nextButton, "setTarget:", aDelegate);
        objj_msgSend(self._nextButton, "setAction:", sel_getUid("_clickArrowNext:"));
...
,["void","CGRect"]), new objj_method(sel_getUid("mouseDown:"), function $_CPDatePickerMonthView__mouseDown_(self, _cmd, anEvent)
{
    ...
    if (objj_msgSend(dayTile, "date").getMonth() == self._date.getMonth())
    {
        if (objj_msgSend(self._datePicker, "datePickerMode") == CPRangeDateMode && objj_msgSend(anEvent, "modifierFlags") & CPShiftKeyMask)
        {
            ...
            if (dateTile < dateValueAtMidnight)
            {
                var interval;
                if (dateTile == dateValueAtMidnight)
                    interval = objj_msgSend(self._datePicker, "timeInterval");
                else
                    interval = objj_msgSend(dateValueAtMidnight, "timeIntervalSinceDate:", dateTile) + objj_msgSend(self._datePicker, "timeInterval");
                objj_msgSend(self._datePicker, "_setDateValue:timeInterval:", objj_msgSend(self, "_hoursMinutesSecondsFromDatePickerForDate:", dateTile), interval);
            }
            else if (objj_msgSend(objj_msgSend(dayTile, "date"), "isEqualToDate:", dateValueAtMidnight))
            {
            ...
            {
                objj_msgSend(self._datePicker, "_setDateValue:timeInterval:", objj_msgSend(self, "_hoursMinutesSecondsFromDatePickerForDate:", objj_msgSend(dateValueAtMidnight, "copy")), objj_msgSend(dateTile, "timeIntervalSinceDate:", dateValueAtMidnight));
            }
            objj_msgSend(self._delegate, "setDateValue:", dateTile);
        }
        else
        {
    ...
    else
    {
        if (self._date.getMonth() - objj_msgSend(dayTile, "date").getMonth() == 1 || self._date.getFullYear() - objj_msgSend(dayTile, "date").getFullYear() == 1)
            objj_msgSend(self._delegate, "_displayPreviousMonth");
        else
            objj_msgSend(self._delegate, "_displayNextMonth");
    }
}
