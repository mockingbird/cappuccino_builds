@STATIC;1.0;I;20;Foundation/CPArray.ji;11;CPControl.ji;20;CPWindow_Constants.ji;21;_CPImageAndTextView.jt;41287;objj_executeFile("Foundation/CPArray.j", NO);objj_executeFile("CPControl.j", YES);objj_executeFile("CPWindow_Constants.j", YES);objj_executeFile("_CPImageAndTextView.j", YES);CPSegmentSwitchTrackingSelectOne = 0;
...
,["BOOL","unsigned"]), new objj_method(sel_getUid("setEnabled:forSegment:"), function $CPSegmentedControl__setEnabled_forSegment_(self, _cmd, shouldBeEnabled, aSegment)
{
    ...
    if (shouldBeEnabled)
        self._themeStates[aSegment] = ThemeState.subtractThemeStates(self._themeStates[aSegment], CPThemeStateDisabled);
    else
        self._themeStates[aSegment] = CPThemeState(self._themeStates[aSegment], CPThemeStateDisabled);
    objj_msgSend(self, "setNeedsLayout");
    objj_msgSend(self, "setNeedsDisplay:", YES);
}
...
{
    if (shouldHighlight)
        self._themeStates[aSegment] = CPThemeState(self._themeStates[aSegment], CPThemeStateHighlighted);
    else
        self._themeStates[aSegment] = ThemeState.subtractThemeStates(self._themeStates[aSegment], CPThemeStateHighlighted);
    objj_msgSend(self, "setNeedsLayout");
    objj_msgSend(self, "setNeedsDisplay:", YES);
}
...
,["CPView","CPString"]), new objj_method(sel_getUid("layoutSubviews"), function $CPSegmentedControl__layoutSubviews(self, _cmd)
{
    ...
    for (var i = 0, count = self._themeStates.length; i < count; i++)
    {
        ...
        if (i == count - 1)
            continue;
        var borderState = CPThemeState(self._themeStates[i], self._themeStates[i + 1]);
...
,["void","int","BOOL"]), new objj_method(sel_getUid("tileWithChangedSegment:"), function $CPSegmentedControl__tileWithChangedSegment_(self, _cmd, aSegment)
{
    ...
    var segment = self._segments[aSegment],
        ...
        themeState = self._themeState.hasThemeState(CPThemeStateDisabled) ? CPThemeState(self._themeStates[aSegment], CPThemeStateDisabled) : self._themeStates[aSegment];
    (contentInset = objj_msgSend(self, "valueForThemeAttribute:inState:", "content-inset", themeState), font = objj_msgSend(self, "font"));
    if (!segmentWidth)
    {
        if (objj_msgSend(segment, "image") && objj_msgSend(segment, "label"))
