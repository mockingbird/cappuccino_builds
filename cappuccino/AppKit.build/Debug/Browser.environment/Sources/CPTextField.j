@STATIC;1.0;i;11;CPControl.ji;17;CPStringDrawing.ji;17;CPCompatibility.ji;21;_CPImageAndTextView.jt;56098;
    ...
    if (!CPTextFieldDOMInputElement)
    {
        ...
        if (CPFeatureIsCompatible(CPInputOnInputEventFeature))
        {
            CPTextFieldInputFunction = function(anEvent)
            {
                ...
                var cappEvent = objj_msgSend(CPEvent, "keyEventWithType:location:modifierFlags:timestamp:windowNumber:context:characters:charactersIgnoringModifiers:isARepeat:keyCode:", CPKeyUp, { x:0.0, y:0.0 }, 0, objj_msgSend(CPEvent, "currentTimestamp"), objj_msgSend(objj_msgSend(CPApp, "keyWindow"), "windowNumber"), nil, nil, nil, NO, nil);
...
{ with(self)
{
    ...
    switch (verticalAlign)
    {
        case CPTopVerticalTextAlignment:
            var topPoint = (contentRect.origin.y) + "px";
            break;

        case CPCenterVerticalTextAlignment:
            var topPoint = ((contentRect.origin.y + (contentRect.size.height) / 2.0) - (lineHeight / 2)) + "px";
            break;

        case CPBottomVerticalTextAlignment:
            var topPoint = ((contentRect.origin.y + contentRect.size.height) - lineHeight) + "px";
            break;
...
{ with(self)
{
    if (objj_msgSend(objj_msgSend(self, "window"), "isKeyWindow") && objj_msgSend(objj_msgSend(self, "window"), "firstResponder") === self)
        objj_msgSend(self, "_becomeFirstKeyResponder");
}
...
{ with(self)
{
    ...
    if (objj_msgSend(self, "_setStringValue:isNewValue:errorDescription:", aValue, NO, function(__input) { if (arguments.length) return error = __input; return error; }) === NO)
    {
        ...
        if (objj_msgSend(_delegate, "respondsToSelector:", sel_getUid("control:didFailToFormatString:errorDescription:")))
            acceptInvalidValue = objj_msgSend(_delegate, "control:didFailToFormatString:errorDescription:", self, aValue, error);
...
class_addMethods(the_class, [new objj_method(sel_getUid("_updatePlaceholdersWithOptions:"), function $_CPTextFieldValueBinder___updatePlaceholdersWithOptions_(self, _cmd, options)
{ with(self)
{
...
},["void","CPDictionary"]), new objj_method(sel_getUid("setPlaceholderValue:withMarker:forBinding:"), function $_CPTextFieldValueBinder__setPlaceholderValue_withMarker_forBinding_(self, _cmd, aValue, aMarker, aBinding)
{ with(self)
{
    objj_msgSend(_source, "setPlaceholderString:", aValue);
...
{ with(self)
{
    objj_msgSend(_source, "setObjectValue:", aValue);
}
},["void","id","CPString"])]);
}
