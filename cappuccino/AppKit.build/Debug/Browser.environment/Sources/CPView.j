@STATIC;1.0;I;20;Foundation/CPArray.jI;23;Foundation/CPGeometry.jI;26;Foundation/CPObjJRuntime.jI;18;Foundation/CPSet.ji;19;CGAffineTransform.ji;9;CPColor.ji;19;CPGraphicsContext.ji;13;CPResponder.ji;9;CPTheme.ji;18;_CPDisplayServer.jt;97333;
...
class_addMethods(the_class, [new objj_method(sel_getUid("toolTip"), function $CPView__toolTip(self, _cmd)
{ with(self)
{
return _toolTip;
}
...
},["void","id"]), new objj_method(sel_getUid("_setupViewFlags"), function $CPView___setupViewFlags(self, _cmd)
{ with(self)
{
...
{ with(self)
{
    ...
    _toolTipFunctionIn = function(e) { objj_msgSend(self, "_fireToolTip"); }
    _toolTipFunctionOut = function(e) { objj_msgSend(self, "_invalidateToolTip"); };
}
...
{ with(self)
{
    ...
    if (self)
    {
        ...
        if (typeof(appkit_tag_dom_elements) !== "undefined" && !!appkit_tag_dom_elements)
            _DOMElement.setAttribute("data-cappuccino-view", objj_msgSend(self, "className"));
...
},["void"]), new objj_method(sel_getUid("_fireToolTip"), function $CPView___fireToolTip(self, _cmd)
{ with(self)
{
    if (CPCurrentToolTipTimer)
    {
        objj_msgSend(CPCurrentToolTipTimer, "invalidate");

        if (CPCurrentToolTip)
            objj_msgSend(CPCurrentToolTip, "close");

        CPCurrentToolTip = nil;
    }

    if (_toolTip)
        CPCurrentToolTipTimer = objj_msgSend(CPTimer, "scheduledTimerWithTimeInterval:target:selector:userInfo:repeats:", CPToolTipDelay, self, sel_getUid("_showToolTip:"), nil, NO);
}
},["void"]), new objj_method(sel_getUid("_invalidateToolTip"), function $CPView___invalidateToolTip(self, _cmd)
{ with(self)
{
    if (CPCurrentToolTipTimer)
    {
...
},["void"]), new objj_method(sel_getUid("_showToolTip:"), function $CPView___showToolTip_(self, _cmd, aTimer)
{ with(self)
{
    if (CPCurrentToolTip)
        objj_msgSend(CPCurrentToolTip, "close");

    CPCurrentToolTip = objj_msgSend(_CPToolTip, "toolTipWithString:", _toolTip);
}
},["void","CPTimer"]), new objj_method(sel_getUid("superview"), function $CPView__superview(self, _cmd)
{ with(self)
{
    return _superview;
}
    ...
    if (_backgroundType !== BackgroundTrivialColor)
    {
        ...
        else
        {
            ...
            if (_backgroundType === BackgroundVerticalThreePartImage)
            {
                ...
                _DOMImageParts[0].style.width = MAX(0.0, ROUND(size.width)) + "px"; _DOMImageParts[0].style.height = MAX(0.0, ROUND(_DOMImageSizes[0].height)) + "px";;
                ...
                _DOMImageParts[2].style.width = MAX(0.0, ROUND(size.width)) + "px"; _DOMImageParts[2].style.height = MAX(0.0, ROUND(_DOMImageSizes[2].height)) + "px";;
            }
            else if (_backgroundType === BackgroundHorizontalThreePartImage)
            {

                _DOMImageParts[0].style.width = MAX(0.0, ROUND(_DOMImageSizes[0].width)) + "px"; _DOMImageParts[0].style.height = MAX(0.0, ROUND(size.height)) + "px";;
                ...
                _DOMImageParts[2].style.width = MAX(0.0, ROUND(_DOMImageSizes[2].width)) + "px"; _DOMImageParts[2].style.height = MAX(0.0, ROUND(size.height)) + "px";;
            }
...
{ with(self)
{
    if (_isHidden || !_hitTests || !(aPoint.x >= (_frame.origin.x) && aPoint.y >= (_frame.origin.y) && aPoint.x < (_frame.origin.x + _frame.size.width) && aPoint.y < (_frame.origin.y + _frame.size.height)))
        return nil;
...
{ with(self)
{
    ...
    else
    {
        ...
        else if (_backgroundType == BackgroundVerticalThreePartImage)
        {
            ...
            _DOMImageParts[0].style.width = MAX(0.0, ROUND(frameSize.width)) + "px"; _DOMImageParts[0].style.height = MAX(0.0, ROUND(_DOMImageSizes[0].height)) + "px";;
            ...
            _DOMImageParts[2].style.width = MAX(0.0, ROUND(frameSize.width)) + "px"; _DOMImageParts[2].style.height = MAX(0.0, ROUND(_DOMImageSizes[2].height)) + "px";;
        ...
        else if (_backgroundType == BackgroundHorizontalThreePartImage)
        {
            ...
            _DOMImageParts[0].style.width = MAX(0.0, ROUND(_DOMImageSizes[0].width)) + "px"; _DOMImageParts[0].style.height = MAX(0.0, ROUND(frameSize.height)) + "px";;
            ...
            _DOMImageParts[2].style.width = MAX(0.0, ROUND(_DOMImageSizes[2].width)) + "px"; _DOMImageParts[2].style.height = MAX(0.0, ROUND(frameSize.height)) + "px";;
...
{ with(self)
{
    ...
    if (self)
    {
        ...
        _hitTests = !objj_msgSend(aCoder, "containsValueForKey:", CPViewHitTestsKey) || objj_msgSend(aCoder, "decodeBoolForKey:", CPViewHitTestsKey);

        objj_msgSend(self, "_setupToolTipHandlers");
        _toolTip = objj_msgSend(aCoder, "decodeObjectForKey:", CPViewToolTipKey);
        if (_toolTip)
            objj_msgSend(self, "_installToolTipEventHandlers");
        ...
        _theme = objj_msgSend(CPTheme, "defaultTheme");
        _themeClass = objj_msgSend(aCoder, "decodeObjectForKey:", CPViewThemeClassKey);
...
{ with(self)
{
    ...
    if (_toolTip)
        objj_msgSend(aCoder, "encodeObject:forKey:", _toolTip, CPViewToolTipKey);
