@STATIC;1.0;I;25;Foundation/CPDictionary.jI;21;Foundation/CPObject.ji;14;CPKeyBinding.ji;12;CPPlatform.jt;8134;objj_executeFile("Foundation/CPDictionary.j", NO);objj_executeFile("Foundation/CPObject.j", NO);objj_executeFile("CPKeyBinding.j", YES);objj_executeFile("CPPlatform.j", YES);var PrimaryPlatformWindow = NULL;
...
,["CGRect"]), new objj_method(sel_getUid("setContentRect:"), function $CPPlatformWindow__setContentRect_(self, _cmd, aRect)
{
    if (!aRect || CGRectEqualToRect(self._contentRect, aRect))
        return;
    self._contentRect = CGRectMakeCopy(aRect);
    self.isa.objj_msgSend0(self, "updateNativeContentRect");
}
...
,["void","id"]), new objj_method(sel_getUid("moveWindow:fromLevel:toLevel:"), function $CPPlatformWindow__moveWindow_fromLevel_toLevel_(self, _cmd, aWindow, fromLevel, toLevel)
{
    if (!aWindow._isVisible)
        return;
    var fromLayer = self.isa.objj_msgSend2(self, "layerAtLevel:create:", fromLevel, NO),
        toLayer = self.isa.objj_msgSend2(self, "layerAtLevel:create:", toLevel, YES);
    (fromLayer == null ? null : fromLayer.isa.objj_msgSend1(fromLayer, "removeWindow:", aWindow));
    (toLayer == null ? null : toLayer.isa.objj_msgSend2(toLayer, "insertWindow:atIndex:", aWindow, CPNotFound));
}
...
{
    self._title = aTitle;
    if (self._DOMWindow && self._DOMWindow.document && (aWindow == null ? null : aWindow.isa.objj_msgSend0(aWindow, "isFullPlatformWindow")))
    {
