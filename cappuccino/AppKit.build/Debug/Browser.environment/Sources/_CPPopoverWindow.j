@STATIC;1.0;I;21;Foundation/CPObject.ji;10;CPButton.ji;8;CPMenu.ji;9;CPPanel.jt;27002;objj_executeFile("Foundation/CPObject.j", NO);objj_executeFile("CPButton.j", YES);objj_executeFile("CPMenu.j", YES);objj_executeFile("CPPanel.j", YES);CPClosableOnBlurWindowMask = 1 << 4;
            ...
            window.setTimeout(function()
            {
                ...
                self._orderFrontTransitionFunction = function()
                {
                    ...
                    self._transitionCompleteFunction = function()
                    {
                        ...
                        self.isa.objj_msgSend2(self, "setCSS3Property:value:", "Transform", nil);
                        self.isa.objj_msgSend2(self, "setCSS3Property:value:", "TransformOrigin", nil);
                        self.isa.objj_msgSend2(self, "setCSS3Property:value:", "Transition", nil);
                        self._isOpening = NO;
...
,["void"]), new objj_method(sel_getUid("orderOut:"), function $_CPPopoverWindow__orderOut_(self, _cmd, aSender)
{
    ...
    if (self._animates && self._browserAnimates)
    {
        if (self._isOpening)
        {
            self._DOMElement.removeEventListener(CPBrowserStyleProperty('transitionend'), self._orderFrontTransitionFunction, YES);
            self._DOMElement.removeEventListener(CPBrowserStyleProperty('transitionend'), self._transitionCompleteFunction, YES);
            var matrix = (window.getComputedStyle(self._DOMElement, null))[CPBrowserStyleProperty("transform")],
                currentScale = ((matrix.split('('))[1].split(','))[0];
            self.isa.objj_msgSend2(self, "setCSS3Property:value:", "Transform", "scale(" + currentScale + ")");
        }
...
,["void","BOOL"]), new objj_method(sel_getUid("_hasOnlyTransientChild:"), function $_CPPopoverWindow___hasOnlyTransientChild_(self, _cmd, aWindow)
{
